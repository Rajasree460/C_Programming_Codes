...
int main(){
	...
	sp *p,t;
	printf("FCFS AT scheduling\n");
	printf("enter the no. of processes: ");
	scanf("%d",&n);
	p=(sp *)malloc(n*sizeof(sp));
	for(i=0;i<n;i++){
		p[i].pid=i+1;
		printf("\nenter the burst time of process id %d: ",p[i].pid);
		scanf("%d",&p[i].bt);
		printf("enter the arrival time of process id %d: ",p[i].pid);
		scanf("%d",&p[i].at);
	}
	
	//sort processes acc to their at
	for(i=0;i<n;i++){
		for(j=i+1;j<n;j++){
			if(p[i].at>p[j].at){
				t=p[i];
